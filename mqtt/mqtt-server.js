...
app.get('/test/workmode',async (req,res)=>{
   for(let item of clients){
    ...
    let downTopic = null
    for(let topic in item.subscriptions){
            if(topic.indexOf('down')>0){
                downTopic = topic
            }
    }
    
    console.log(downTopic);
    sendWorkMode(item,downTopic);
   }
    res.status(200).json({
        msg:"调用成功"
    });
...
app.post('/test/serverInfo',async (req,res)=>{
   ...
   for(let item of clients){
    ...
    console.log(downTopic);
    sendServerInfo(item,downTopic,serverInfo);
   }
    res.status(200).json({
        msg:"调用成功"
    });
...
app.get('/test/all/topics',async(req,res)=>{
    for(let item of clients){
        for(let topic in item.subscriptions){
            console.log("client_id:"+item.id+";   "+topic);
        }
    }
    res.status(200).json({
        msg:"调用成功"
...
aedesInstance.on('unsubscribe',function(subscriptions,client){
    for(let item of subscriptions){
        console.log("当前取消订阅:"+item+";   client:"+client.id);
    }
});
...
function sendWorkMode(client,topic){
    aedesInstance.publish({
        topic:topic,
        payload:Buffer.from(`{"msgid":"${randomUUID()}","cmd":"workMode","mode":"write","data":"0"}`),
        qos:1,
        retain:false
    },(err)=>{
        if (err) {
...
function sendServerInfo(client,topic,serverInfo){
    aedesInstance.publish({
        topic:topic,
        payload:Buffer.from(`{"msgid":"${randomUUID()}","cmd":"serverInfo","mode":"write","data":"${serverInfo}"}`),
        qos:1,
        retain:false
    },(err)=>{
        if (err) {
...
function sendGetData(client,topic){
    aedesInstance.publish({
        topic:topic,
        payload:Buffer.from(`{"msgid":"${randomUUID()}","cmd":"getData","mode":"read"}`),
        qos:1,
        retain:false
       },
       (err)=>{
